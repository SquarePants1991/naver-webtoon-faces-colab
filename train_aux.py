...
def batch_vertical_concat(imgs):
    return torch.cat([img for img in imgs], 2)
...
def train(
    ...
    loader_aux = sample_data(loader_aux)
    ...
    for idx in pbar:
        ...
        real_img_aux = next(loader_aux)
        real_img_aux = real_img_aux.to(device)
        ...
        structure_aux, _ = encoder(real_img_aux)

        fake_img1 = generator(structure1, texture1)
        fake_img2 = generator(structure1, texture2)
        
        fake_img_aux = generator(structure_aux, texture2)
        
        fake_pred = discriminator(torch.cat((fake_img1, fake_img2, fake_img_aux), 0))
        ...
        fake_patch_aux = patchify_image(fake_img_aux, args.n_crop)

        ref_patch = patchify_image(real_img2, args.ref_crop * args.n_crop)
        fake_patch_pred, ref_input = cooccur(
            torch.cat((fake_patch, fake_patch_aux), 0), ref_patch, ref_batch=args.ref_crop
        )
        ...
        recon_loss = F.l1_loss(fake_img1, real_img1) # + 0.2 * F.l1_loss(fake_img_aux, real_img_aux)
        ...
        if get_rank() == 0:
            ...
            if i % 100 == 0:
                with torch.no_grad():
                    ...
                    structure_aux, _ = e_ema(real_img_aux)

                    fake_img1 = g_ema(structure1, texture1)
                    fake_img2 = g_ema(structure1, texture2)
                    fake_img_aux = g_ema(structure_aux, texture2)

                    sample = torch.cat([
                        torch.cat([img.unsqueeze(0) for img in batch_imgs], 2) 
                        for batch_imgs in [fake_img1, fake_img2, real_img2, fake_img_aux, real_img_aux]
                    ], 3)
...
if __name__ == "__main__":
    ...
    assert args.batch > 1 and args.batch % 2 == 0
    
    dataset_aux = MultiResolutionDataset(args.path_aux, transform, args.size)
    loader_aux = data.DataLoader(
        dataset_aux,
        batch_size=args.batch//2,
        sampler=data_sampler(dataset_aux, shuffle=True, distributed=args.distributed),
        drop_last=True,
    )
